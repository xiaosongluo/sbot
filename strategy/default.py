...
async def default_handler(event):
    ...
    if any(keyword in content for keyword in KEY_WORDS):
        markdown_text = _format_message(content)
        await _send_notification(markdown_text)
    else:
...
def _parse_message(event):
    ...
    content.extend(_parse_media(msg))
    return "\n".join(content)


def _parse_media(msg):
    """解析多媒体内容"""
    media_info = []
    if msg.media:
...
async def _send_notification(markdown_text):
    """发送钉钉通知"""
    try:
